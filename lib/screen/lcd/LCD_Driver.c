...
void LCD_Init(LCD_SCAN_DIR LCD_ScanDir, uint16_t LCD_BLval)
{
	...
	if(LCD_BLval > 1000) LCD_BLval = 1000;
...
parameter:
	...
	xEnd   :   X direction end coordinates
********************************************************************************/
void LCD_SetCursor(POINT Ypoint, POINT Xpoint)
{
	LCD_SetWindow(Xpoint, LCD_2_8_HEIGHT-Ypoint, Xpoint, LCD_2_8_HEIGHT-Ypoint);  //RH 20220512
}
