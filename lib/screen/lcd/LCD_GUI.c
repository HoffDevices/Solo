...
void GUI_DisCharScale(POINT Xpoint, POINT Ypoint, const char Acsii_Char, sFONT* Font, COLOR Color_Background, COLOR Color_Foreground, int Scale)
{
    POINT Page, Column;
    ...
    for(Page = 0; Page < Font->Height; Page ++ ) {
        for(Column = 0; Column < Font->Width; Column ++ ) {
            ...
            if(FONT_BACKGROUND == Color_Background) { //this process is to speed up the scan
                if(*ptr & (0x80 >> (Column % 8)))
                    GUI_DrawPoint(Xpoint + Column * Scale, Ypoint + Page * Scale, Color_Foreground, DOT_PIXEL_2X2, DOT_STYLE_DFT);
            } else {
                if(*ptr & (0x80 >> (Column % 8))) {
                    GUI_DrawPoint(Xpoint + Column * Scale, Ypoint + Page * Scale, Color_Foreground, DOT_PIXEL_2X2, DOT_STYLE_DFT);
                } else {
                    GUI_DrawPoint(Xpoint + Column * Scale, Ypoint + Page * Scale, Color_Background, DOT_PIXEL_2X2, DOT_STYLE_DFT);
                }
            }
            //One pixel is 8 bits
            if(Column % 8 == 7)
